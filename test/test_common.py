...
from utils.common.setup import load_weights_to_gpu,\
    config_to_string, cal_quat_angle_error,\
    make_deterministic, lprint
...
def test_result():
    ...
    docstring
    """
    error_result = cal_quat_angle_error(np.array([[0, 1], [2, 3], [4, 5]]),
                                        np.array([[0, 1], [2, 3], [4, 5]]))
    pred_result = np.array([[0.], [0.], [0.]])

    np.testing.assert_almost_equal(error_result, pred_result)

    error_result = cal_quat_angle_error(np.array([[0.1, 0.2], [0.2, 0.3],
                                                  [0.4, 0.5]]),
                                        np.array([[0.2, 0.3], [1000, 1000],
                                                  [1000, 1000]]))
...
class SetupTestCase(unittest.TestCase):
    ...
    def test_types(self):
        ...
        self.assertIsNone(load_weights_to_gpu(),
                          'load_weights_to_gpu from utils.common.setup '
                          'is not None without args')

        self.assertIsNone(lprint(''),
                          'lprint from utils.common.setup return not None')

        self.assertIsInstance(cal_quat_angle_error
                              (np.array([[0, 1], [2, 3], [4, 5]]),
    ...
    def test_exceptions(self):
        ...
        with self.assertRaises(ValueError):
            cal_quat_angle_error(np.array([[0, 1], [2, 3]]),
                                 np.array([[0, 1], [2, 3], [4, 5]]))
